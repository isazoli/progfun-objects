...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  val ROOT = new Tweet("","",Int.MinValue);

  override def filter(p: Tweet => Boolean) : TweetSet = filterAcc(p, new NonEmpty(ROOT, new Empty, new Empty))
  ...
  override def union(that : TweetSet) = {
    ...
    foreach(_ => new NonEmpty(_, Empty, Empty))
  }
}
